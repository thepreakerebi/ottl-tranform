...
export default function RenameAttributeBlockConfiguration({ signal, description: helpText, onApply, onCancel, initialConfig }: Props) {
  ...
  useEffect(() => {
    ...
    if (typeof ic["newKey"] === "string") setNewKey(ic["newKey"] as string);
    if (ic["condition"]) {
  ...
  const canSave = keys.length > 0 && newKey.trim().length > 0 && (scope !== "conditional" || !!(condition && validateChain(condition)));
  ...
  function handleApply() {
    const summary = buildSummary(scope, keys, newKey, condition || undefined);
    const config = { scope, keys, newKey, condition };
    onApply(summary, config);
  }

  return (
    <section aria-label="Configure rename attribute" className="w-full max-w-full flex flex-col">
      <PopoverHeader className="flex items-start justify-between">
        <section>
          <h3 className="text-sm font-semibold">Rename attribute</h3>
          <p className="text-xs text-muted-foreground">{helpText || "Rename selected attribute keys on the chosen level"}</p>
        </section>
      ...
      <PopoverBody className="max-h-[300px] space-y-4">
        ...
        <section className="space-y-2">
          <Label className="text-xs font-medium">Attribute Key(s) to Rename</Label>
        ...
        <Separator />
        <section className="space-y-2">
          <Label className="text-xs font-medium" htmlFor="new-key">New attribute key name</Label>
          <Input id="new-key" value={newKey} onChange={(e) => setNewKey(e.target.value)} placeholder="e.g. http.status_code" />
        </section>
      </PopoverBody>
      <PopoverSeparator />
...
function buildSummary(scope: ScopeValue, keys: string[], newKey: string, condition?: ConditionChain) {
  const base = `Rename ${keys.length} ${keys.length === 1 ? "attribute" : "attributes"} to ${newKey} on ${scope}`;
  if (condition) {
