...
export default function MaskAttributeBlockConfiguration({ signal, description: helpText, onApply, onCancel, initialConfig }: Props) {
  ...
  // Clear selections when scope changes to avoid stale keys
  useEffect(() => {
    setKeys([]);
    setQuery("");
    setComboOpen(false);
  ...
  useEffect(() => {
    ...
    if (typeof ic["substringStart"] === "string") setStart(ic["substringStart"] as string);
    if (typeof ic["substringEnd"] === "string") setEnd(ic["substringEnd"] as string);
    if (ic["condition"]) {
  ...
  const endNum = end.trim().length ? Number(end) : undefined;
  const substringValid = Number.isFinite(startNum) && startNum >= 0 && (endNum === undefined || (Number.isFinite(endNum) && (endNum as number) >= startNum));
  const canSave = hasValidKeys && substringValid && (scope !== "conditional" || !!(condition && validateChain(condition)));
  ...
  function handleApply() {
    const summary = buildSummary(scope, keys, start, end, condition || undefined);
    const config = { scope, keys, substringStart: start, substringEnd: end, condition };
    onApply(summary, config);
  }

  return (
    <section aria-label="Configure mask attribute" className="w-full max-w-full flex flex-col">
      <PopoverHeader className="flex items-start justify-between">
        <section>
          <h3 className="text-sm font-semibold">Mask attribute</h3>
          <p className="text-xs text-muted-foreground">{helpText || "Mask a substring of selected attribute values with *"}</p>
        </section>
      ...
      <PopoverBody className="max-h-[300px] space-y-4">
        ...
        <section className="space-y-2">
          <Label className="text-xs font-medium">Attribute Key(s) to Mask</Label>
          ...
          {keys.length > 0 && (
            <section className="text-[11px] text-muted-foreground">Selected: {keys.join(", ")}</section>
          )}
        </section>

        <Separator />
        <section className="space-y-2">
          <Label className="text-xs font-medium">Substring to mask</Label>
          <section className="flex items-center gap-2">
            <Input value={start} onChange={(e) => setStart(e.target.value)} placeholder="start" className="w-24" />
            <Input value={end} onChange={(e) => setEnd(e.target.value)} placeholder="end (optional)" className="w-32" />
          </section>
          <p className="text-[11px] text-muted-foreground">Selected substring will be replaced with * characters.</p>
        </section>
      </PopoverBody>
      <PopoverSeparator />
...
function buildSummary(scope: ScopeValue, keys: string[], start: string, end: string, condition?: ConditionChain) {
  const range = end && end.trim().length ? `${start}:${end}` : `${start}:`;
  const base = `Mask ${keys.length} ${keys.length === 1 ? "attribute" : "attributes"} on ${scope} substring [${range}]`;
  if (condition) {
